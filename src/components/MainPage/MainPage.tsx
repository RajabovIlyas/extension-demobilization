...
import {useUpdateDate} from "./update-date.hook.ts";
...
const MainPage: FC<MainPageProps> = ({setPath}) => {
    const {remainingDays} = useUpdateDate();

    const onSetting = () => {
        setPath(PathEnum.SETTINGS);
    }

    const showDate = remainingDays ? (
        <h1 className="pb-2">
            {remainingDays}
        </h1>
    ) : (
        <button className="btn" onClick={onSetting}>
            Insert Date
        </button>
    );
